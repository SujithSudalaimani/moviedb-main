...
const MovieAbout = () => {
  ...
  const fetchMovieDetails = async (title) => {
    try {
      const response = await axios.get("/.netlify/functions/fetchMovies", {
        params: {
          title: title, // your serverless function will map this to `t`
        },
      });
      ...
      setErrorMsg("Could not load movie details. Please try again.");
    }
  };
  ...
  if (errorMsg) {
    return (
      <div className="p-10 text-red-500 text-center">{errorMsg}</div>
    );
  }
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 flex flex-col md:flex-row w-full max-w-5xl border border-white">
        ...
        <div className="flex-1 text-gray-800 space-y-3 pr-6">
          ...
          <p className="text-sm text-gray-600">
            ...
            {movieDetails.Released?.split(" ").slice(2).join(" ")}
          </p>
